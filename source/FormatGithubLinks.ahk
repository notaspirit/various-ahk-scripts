...
#+q:: {
    ...
    if RegExMatch(url, "https://github\.com/([^/]+)/([^/]+)/(issues|pull)/(\d+)", &m) {
    ...
    } else if RegExMatch(url, "https://github\.com/([^/]+)/([^/]+)/milestone/(\d+)", &m) {
        ; Milestone link
        owner := m[1]
        repo := m[2]
        number := m[3]
        title := ""
        try {
            apiUrl := "https://api.github.com/repos/" . owner . "/" . repo . "/milestones/" . number
            req := ComObject("WinHttp.WinHttpRequest.5.1")
            req.Open("GET", apiUrl, false)
            req.SetRequestHeader("User-Agent", "AHK-Script")
            req.SetRequestHeader("Accept", "application/vnd.github+json")
        ...
        if (title != "")
            formatted := "[" . owner . "/" . repo . ": " . title . " Milestones](<" . url . ">)"
        else
            formatted := "[" . owner . "/" . repo . ": Milestone " . number . "](<" . url . ">)"
        A_Clipboard := formatted
        ToolTip("✓ Transformed to: " . formatted)
